        ...
        {filtered.map((e) => {
          ...
          return (
            <div
              ...
              className="rounded-2xl border border-[#E2E8F0] bg-white p-5 card-hover cursor-pointer shadow-xs active:scale-[0.99] transition-all"
              onClick={() => prod.id && navigate(`/ledger?product=${prod.id}`)}
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-[#EBF5FA] flex items-center justify-center shrink-0 text-[#0091FF]">
                  <Boxes className="w-6 h-6" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <span className="font-semibold text-base text-[#010B1C]">{prod.name || "Material Item"}</span>
                    <span className="font-mono text-xs font-semibold text-[#0091FF] bg-[#EBF5FA] px-2 py-0.5 rounded-md border border-[#98CAE4]/40">{prod.sku || "SKU"}</span>
                    {prod.category_name && (
                ...
                <div className="text-right shrink-0">
                  ...
                  {e.negative && <Badge className="mt-1 bg-rose-50 text-rose-700 border-rose-200 text-[10px]">Discrepancy</Badge>}
