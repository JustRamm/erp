  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-[#E2E8F0] bg-white shadow-xs overflow-hidden divide-y divide-[#F1F5F9]">
        {users.map((u) => (
          <div key={u.id} data-testid={`user-row-${u.email}`} className="flex items-center gap-3.5 px-5 py-4 hover:bg-[#F8FAFC] transition-colors">
            ...
            <div className="flex items-center gap-3 ml-2">
              ...
              {u.role !== "admin" && (
                <button onClick={() => del(u)} data-testid={`del-user-${u.email}`} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input data-testid="user-password-input" type="text" value={f.password} onChange={(e) => setF({ ...f, password: e.target.value })} className="mt-1.5 h-11 font-mono" placeholder="••••••••" />
            </div>
            <div>
              <Label className="text-xs font-semibold uppercase tracking-wider text-slate-600">System Role</Label>
              <Select value={f.role} onValueChange={(v) => setF({ ...f, role: v })}>
                <SelectTrigger data-testid="user-role-select" className="mt-1.5 bg-white border-[#E2E8F0] h-11"><SelectValue /></SelectTrigger>
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button onClick={create} data-testid="save-user-btn">Create User Account</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
