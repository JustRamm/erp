...
function Big({ icon: Icon, value, label, unit, color, bg, delay }) {
  return (
    <div
      className="rounded-2xl border border-[#E2E8F0] bg-white p-6 card-hover rise shadow-[0_4px_20px_-2px_rgba(0,145,255,0.06),0_2px_6px_-1px_rgba(1,11,28,0.03)]"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <Big icon={Waves} value={d?.plastic_diverted_kg ?? 0} unit="kg" label="Ocean Plastic Processed" color="text-[#0091FF]" bg="bg-sky-50" delay={0} />
        <Big icon={Leaf} value={d?.co2_saved_kg ?? 0} unit="kg" label="CO₂ Emissions Offset" color="text-emerald-600" bg="bg-emerald-50" delay={80} />
        <Big icon={Factory} value={production.reduce((a, b) => a + b.qty, 0)} unit="units" label="Circular Products Built" color="text-amber-700" bg="bg-amber-50" delay={160} />
        <Big icon={MapPin} value={d?.deployed_count ?? 0} unit="sites" label="Public Municipal Deployments" color="text-indigo-600" bg="bg-indigo-50" delay={240} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-[0_4px_20px_-2px_rgba(0,145,255,0.06),0_2px_6px_-1px_rgba(1,11,28,0.03)]">
          ...
          {production.length === 0 && <p className="text-slate-400 text-sm text-center py-8">No production output logged yet.</p>}
        </div>
        ...
        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-[0_4px_20px_-2px_rgba(0,145,255,0.06),0_2px_6px_-1px_rgba(1,11,28,0.03)]">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={volumes} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} innerRadius={60} paddingAngle={3}>
